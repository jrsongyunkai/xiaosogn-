<template>
  <div id="linkageScene">
    <div class="Scene">
      <Row class="header">
        <span>联动配置</span>
        <Button class="b-blue" @click="handleLinkageScene"
          >联动新增</Button
        >
      </Row>
      <Col>
        <Table :data="linkageData" :columns="linkagecolumns" :height="520" :max-height="620">
          <template slot-scope="{ row }" slot="name">
            <span class="pointer c-default" @click="handleViewScene(row)">{{
              row.name
            }}</span>
          </template>
          <template slot-scope="{ row }" slot="status">
            <span v-if="row.status === 2" class="b-danger">禁用</span>
            <span v-else class="b-success">启用</span>
          </template>
          <template slot-scope="{ row }" slot="operating">
            <Button
              class="top-padding-bottom left-padding-right c-primary"
              type="text"
              @click="handleLinkageScene(row, 'edit')"
              >编辑</Button
            >
            <Button
              class="top-padding-bottom left-padding-right c-primary"
              type="text"
              @click="handleLinkagedeLock(row)"
              >停启</Button
            >
            <Button
              class="top-padding-bottom left-padding-right c-danger"
              type="text"
              @click="handleSceneDel(row.id)"
              >删除</Button
            >
          </template>
        </Table>
      ...
      <Form :model="preData">
        ...
        <div class="dis_f">
          ...
          <Tag
            ...
            :key="index"
            closable
            @on-close="handleCloseTag('pre', tag)"
          >
            {{ tag.title }}
          </Tag>
        </div>
      ...
      <Form :model="actionData">
        ...
        <FormItem v-show="actionData.mac">
          ...
          <Tag
            ...
            :key="index"
            closable
            @on-close="handleCloseTag('action', tag)"
          >
            {{ tag.title }}
          </Tag>
          </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            changePages (e) { this.queryLinkageSceneList(e); },
            init () {
