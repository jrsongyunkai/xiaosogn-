<template>
  <div id="NewGrouping">
    <div style="margin-top:10px; background: #1a202e;padding:20px;height:750px">
    ...
    >
      <template slot-scope="{ row, index }" slot="name" >
        ...
        <br>
        <span :style="row.num===1?'float:left;color:#fff;text-align:center;margin:2px;':'float:left;color:#fff;text-align:center;margin:2px;margin-top: -22px;margin-left:'+ (10 + row.num*15) +'px'" >[{{ row.num}}]</span>
        <Input
          type="text"
          :style="row.num===1?'float:left;width:139px;':'float:left;width:139px;margin-top:-25px'"
          v-model="row.name"
          :value="20 + (row.num*2)"
          :disabled='isdisa'
      ...
      <template slot-scope="{ row,index }" slot="counts">
        <span style="cursor:pointer;margin-left:20px" @click="selectEquip(row, index)">{{ row.devices ? row.devices.length : 0 }}</span>
      </template>
        ...
        <Form ref="formValidate" :model="row" :rules="ruleValidate" >
            <FormItem prop="area">
                <Input
                    type="text"
                    v-model="row.area"
                    style="width:140px;margin-top:23px"
                    :disabled='isdisa'
                    @on-enter="enter(row, index, 1)"
                    @on-blur="enter(row, index, 1)"
                />
            </FormItem>
        </Form>
           ...
           <Form ref="formValidate" :model="row" :rules="ruleValidate" >
            <FormItem prop="people">
                <Input
                  ...
                  style="width:140px;margin-top:23px"
                  :disabled='isdisa'
                  @on-enter="enter(row, index, 2)"
                  @on-blur="enter(row, index, 2)"
                />
            </FormItem>
        </Form>
      ...
      <template slot-scope="{ row, index }" slot="longitude">
          <Form ref="formValidate" :model="row" :rules="ruleValidate" >
            <FormItem prop="longitude">
                <Input
                  ...
                  :disabled='isdisa'
                  @on-enter="enter(row, index, 3)"
                  @on-blur="enter(row, index, 3)"
                />
            </FormItem>
        </Form>
      ...
      <template slot-scope="{ row, index }" slot="latitude">
          <Form ref="formValidate" :model="row" :rules="ruleValidate" >
            <FormItem prop="latitude">
                <Input
                  ...
                  :disabled='isdisa'
                  @on-enter="enter(row, index, 4)"
                  @on-blur="enter(row, index, 4)"
                />
            </FormItem>
        </Form>
      </template>
      ...
      <template slot-scope="{ row, index }" slot="action">
        <div>
          <Dropdown @on-click="addLv($event,row, index)" >
            <Button  class="edit" type="text" size="small" style="margin-right: 5px;color:#007EFF" :disabled='isdisa' >
              添加分组
            </Button>
            <DropdownMenu slot="list" >
              <!-- <DropdownItem name = '添加上级'>添加上级</DropdownItem> -->
              <DropdownItem name = '添加本级' :disabled='isdisa'>添加本级</DropdownItem>
              <DropdownItem name = '添加下级' :disabled='isdisa'>添加下级</DropdownItem>
            </DropdownMenu>
          </Dropdown>
          ...
          <Poptip
            ...
            cancel-text="取消"
          >
            <Button class="delete" type="text" size="small"  style="margin-right: 5px;color:#E03C3A " :disabled='isdisa'>删除分组</Button>
          </Poptip>
          <Button
            class="choose"
            type="text"
    ...
    export default {
        data () {
            return {
                ...
                ruleValidate: {
                    area: [
                        { required: false, type: 'string', pattern: /^(([1-9]\d{0,3})|0)(\.\d{0,2})?$/, message: '请输入正确格式', trigger: 'change' }
                    ],
                    people: [
                        { required: false, type: 'string', pattern: /^\d+$/, message: '请输入正确格式', trigger: 'change' }
                    ],
                    longitude: [
                        { required: false, type: 'string', pattern: /^(([1-9]\d{0,3})|0)(\.\d{0,2})?$/, message: '请输入正确格式', trigger: 'change' }
                    ],
        ...
        methods: {
            ...
            handleEdit (row, index) { // 添加下级
                if (row.num === 7) {
                    this.$Message.error('最多只能添加7级分组哦')
                    return false
                }
                function sys (id, arr) {
                    let flag = false;
